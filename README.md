...
little modify to this project to make it works with 4 digits captcha identify on centos8:

- remove invitation code/email verfiy(so you can register with any invitation)
- other code modify in /model_server/ for support on tensorflow2 and 4 digits captcha identify.
- minimum the needs of python packages.

the other setup step pls follow the official docs(https://github.com/VKSRC/captcha_orz).
...
6:1234,
```
means the result was processed by model 6,and this model identified 1234 in the picture you upload.

besides,this repo added venv support u can use by ``source /bin/active`` but not necessary.and this may cause compatibility problem in your environment.so you better follow the offical docs step to install.

if you want to train a new captcha code,simplfy step will be:
